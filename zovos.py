...
class Window:
    def __init__(self, title, width, height, x, y, file=None, settings_app=None, browser_app=None, calculator_app=None,
        ...
        self.ztext_app = ztext_app  # ZText App
        self.ztable_app = ztable_app  # ZTable App
        self.zdb_app = zdb_app  # ZDataBase App
    ...
    def draw(self, screen):
        if self.is_open:
            ...
            elif self.apvia_app:  # Draw Apvia app
                draw_rect(screen, window_color, self.rect)
            ...
            elif self.ztext_app:  # Draw ZText App
                draw_rect(screen, window_color, self.rect)
                self.ztext_app.width = self.width
                self.ztext_app.height = self.height
                self.ztext_app.draw(screen.subsurface(self.rect))
            elif self.ztable_app:  # Draw ZTable App
                draw_rect(screen, window_color, self.rect)
                self.ztable_app.width = self.width
                self.ztable_app.height = self.height
                self.ztable_app.draw(screen.subsurface(self.rect))
            elif self.zdb_app:  # Draw ZDataBase App
                draw_rect(screen, window_color, self.rect)
                self.zdb_app.width = self.width
                self.zdb_app.height = self.height
                self.zdb_app.draw(screen.subsurface(self.rect))
...
class DesktopFile:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.dragging:
                    for dragged_file in dragged_files:  # Move all dragged files together
                        dragged_file.rect.center = (
                        event.pos[0] + (dragged_files[0].rect.centerx - self.rect.centerx), event.pos[1] + (
    ...
    def open_file(self, files, windows, taskbar):
        ...
        elif self.is_app and self.name == "апвыа":  # Open Apvia App
            if windows:
                last_window = windows[-1]
                new_x = last_window.title_bar.x + 30
                new_y = last_window.title_bar.y + 30
        ...
        elif self.is_app and self.name == "ZText":  # Open ZText App
            if windows:
                last_window = windows[-1]
                new_x = last_window.title_bar.x + 30
                new_y = last_window.title_bar.y + 30
        ...
        elif self.is_app and self.name == "ZTable":  # Open ZTable App
            if windows:
                last_window = windows[-1]
                new_x = last_window.title_bar.x + 30
                new_y = last_window.title_bar.y + 30
        ...
        elif self.is_app and self.name == "ZDataBase":  # Open ZDataBase App
            if windows:
                last_window = windows[-1]
                new_x = last_window.title_bar.x + 30
                new_y = last_window.title_bar.y + 30
...
files.append(DesktopFile(  # Apvia App Icon
    name="апвыа",
    image_path="авпыашка.png",  # Make sure you have py.png in images/
    x=10, y=450,
    is_app=True
))
files.append(DesktopFile(  # Apvia App Icon
    name="ZText",
    image_path="ztext.png",  # Make sure you have py.png in images/
    x=110, y=250,
    is_app=True
))
files.append(DesktopFile(  # Apvia App Icon
    name="ZTable",
    image_path="ztable.png",  # Make sure you have py.png in images/
    x=110, y=350,
    is_app=True
))
                ...
                if selected_file and not selected_file.protected:
                    ...
                    if folder_window_instance:  # Right click in folder window background
                        ...
                        context_menu_options = ContextMenu(
                            ...
                            event.pos[0], event.pos[1], ["Создать .txt", "Создать папку", "Обновить"], on_desktop=False,
                            folder_window=folder_window_instance)
                    else:  # Right click on desktop background
                        print('ne ahui')
                        context_menu_options = ContextMenu(
                            event.pos[0], event.pos[1], ["Создать .txt", "Создать папку", "Обновить"], on_desktop=True,
                            folder_window=folder_window_instance)
                elif trash.selection_rect.collidepoint(event.pos):
                    context_menu_options = ContextMenu(
                        event.pos[0], event.pos[1], ["Очистить корзину"], on_desktop=True,
                        folder_window=folder_window_instance)
                else:
                    context_menu = None
            elif event.button == 1:
